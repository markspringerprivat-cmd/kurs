...
if (!CSS.escape) CSS.escape = value => String(value).replace(/[^a-zA-Z0-9_-]/g, '\\$&');
...
function renderSavedPreviews() {
  document.querySelectorAll('[data-display-from]').forEach(box => {
    const value = loadValue(box.dataset.displayFrom, '').trim();
    box.textContent = value || box.textContent || 'Noch nichts gespeichert.';
  });
}
...
function saveListItems(key, items) {
  saveValue(key, items);
}

function renderList(key) {
  document.querySelectorAll(`[data-list-display="${CSS.escape(key)}"]`).forEach(target => {
    const items = listItems(key);
    if (!items.length) {
      target.innerHTML = '<p class="empty-state">Noch keine Einträge gespeichert.</p>';
      return;
    }
    target.innerHTML = items.map((item, index) => `
      <div class="statement-item">
        <span>${escapeHtml(item)}</span>
        <button type="button" data-remove-list-item="${escapeHtml(key)}" data-index="${index}" aria-label="Eintrag entfernen">×</button>
      </div>
    `).join('');
  });
}

function setupRoleStepLists() {
  const forms = document.querySelectorAll('[data-list-form]');
  forms.forEach(form => {
    const key = form.dataset.listForm;
    const input = form.querySelector('[data-list-input]');
    renderList(key);
    form.addEventListener('submit', event => {
      event.preventDefault();
      const value = (input?.value || '').trim();
      ...
      const items = listItems(key);
      items.push(value);
      saveListItems(key, items);
      input.value = '';
      renderList(key);
    });
  });

  document.querySelectorAll('[data-list-display]').forEach(target => renderList(target.dataset.listDisplay));

  document.addEventListener('click', event => {
    const removeButton = event.target.closest('[data-remove-list-item]');
    if (removeButton) {
      const key = removeButton.dataset.removeListItem;
    ...
    if (chip) {
      const form = chip.closest('.step-workspace')?.querySelector('[data-list-form]');
      const input = form?.querySelector('[data-list-input]');
      if (input) {
